...
classDiagram
    class Persona {
        ...
        +int telefono
        +dni() str
        +validar_dni(DNI) bool
        +validar_movil(telefono) bool
    }
    ...
    class Alquiler {
        ...
        +iniciar_alquiler(fecha) bool
        +finalizar_alquiler(fecha) bool
    }

    class GestionCliente {
        +list clientes
        +añadir_cliente(...) bool
        +elminar_cliente(dni) bool
        +buscar_cliente(dni) Cliente
        +añadir_metodo_pago(dni, metodo) bool
        +eliminar_metodo_pago(dni, metodo) bool
    }

    class GestionTrabajador {
        +list trabajadores
        +contratar(cargo, ...) bool
        +despedir(dni) bool
        +buscar_trabajador(dni) Trabajador
    ...
    class GestionSede {
        ...
        +añadir_sede(...) bool
        +añadir_coche(...) bool
        +añadir_furgoneta(...) bool
        +añadir_moto(...) bool
        +eleminar_vehiculo(...) bool
        +mover_vehiculo(...) bool
        +anadir_trabajador(...) bool
        +eliminar_trabajador(...) bool
        +lista_vehiculos_disponibles(id) list
        +lista_vehiculos_ocupados(id) list
    }

    class GestionMantenimiento {
        +list vehiculos_averiados
        +añadir_avería(averia, matricula) bool
        +reparar_vehiculo(matricula) float
        +calcular_coste(matricula) float
    }
    ...
    class GestionAlquiler {
        ...
        +crear_alquiler(...) bool
        +crear_reserva(...) bool
        +buscar_alquiler_codigo(cod) Alquiler
        +buscar_reserva_solapada(...) Reserva
        +eliminar_reserva(...) bool
    }
    ...
    Cliente "0..*" --> "0..*" Vehiculo : vehiculos activos
    Alquiler --> Cliente
    Alquiler --> Vehiculo
    Alquiler --> Trabajador
    Vehiculo "1" o-- "0..*" Reserva : reservas

    Sede "1" o-- "0..*" Vehiculo : inventario
    Sede "1" o-- "0..*" Trabajador : plantilla

    GestionCliente --> Cliente
    GestionTrabajador --> Trabajador
    GestionSede --> Sede
    GestionMantenimiento --> GestionSede
    GestionAlquiler --> GestionCliente
    GestionAlquiler --> GestionSede
    GestionAlquiler --> GestionTrabajador

    Menu --> GestionCliente
    Menu --> GestionTrabajador
    Menu --> GestionSede
    Menu --> GestionMantenimiento
    Menu --> GestionAlquiler
```
